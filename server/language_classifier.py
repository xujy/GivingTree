...
class LanguageClassifier():
    ...
    def classify(self, docdata):
        string = " ".join(docdata['concepts'])
        classes = self.natural_language_classifier.classify('c533b1x244-nlc-3169', string)
        relevant_class_name = max(classes['classes'], key=lambda x: x['confidence'])
        return relevant_class_name['class_name']
